...
function minimize(obj, cleanArrays = false) {
  const keys = Object.keys(obj);
  let i = keys.length;
  let hasKeys;
  let key;
  ...
  while (i--) {
    ...
    if (utils.isObject(obj[key]) && !Buffer.isBuffer(obj[key])) {
      obj[key] = minimize(obj[key], true);
      if (!obj[key]) {
        delete obj[key];
        continue;
      }
    ...
    if (cleanArrays && Array.isArray(obj[key]) && obj[key].length === 0) {
      delete obj[key];
      continue;
    }
